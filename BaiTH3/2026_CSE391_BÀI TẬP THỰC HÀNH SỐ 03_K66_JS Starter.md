...
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
  Khi trang web được tải, trình duyệt sẽ chạy file JavaScript và thực thi các lệnh bên trong/
  Trong trường hợp này, JavaScript đã in ra "Hello from Javascript" vào Console
...
> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?)
  JavaScript có thể chạy trên cả trình duyệt và trên server
    + Trong trình duyệt, JavaScript được dùng để xử lý tương tác với người dùng
      và thay đổi nội dung trang web
    + Trên server, JavaScript có thể chạy bằng môi trường như Node.js
      để xử lý dữ liệu và logic phía máy chủ
...
> - JavaScript: Dùng để tạo tính năng và tương tác động cho trang web (xử lý sự kiện, thay đổi nội dung, kiểm tra dữ liệu, cập nhật trang mà không cần tải lại)
...
> - Dòng log hiển thị gì sau khi em sửa thông tin?
  Dòng log hiển thị là "Xin chào, mình là Ly, năm nay mình 20 tuổi."
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
  + Nếu quên dấu ';', trong nhiều trường hợp JavaScript vẫn có thể chạy vì JavaScript có cơ chế tự chèn dấu chấm phảy.
    Trong 1 số trường hợp đặc biệt có thể gây lỗi hoặc kết quả không mong muốn
  + Nếu quên dấu + khi nối chuỗi, JavaScript sẽ báo lỗi cú pháp trong Console:
...
> - Điều thú vị nhất em vừa khám phá được về console là gì?
  Console có thể hiển thị kết quả của JavaScript ngay lập tức
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
  Em gặp lỗi cú pháp quên dấu + khi nối chuỗi trong console.log. Em tự sửa
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: Khớp
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: Khớp
...
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
  Chương trình sẽ kiểm tra từng điều kiện theo thứ tự từ trên xuống dưới. Nếu điều kiện đúng thì thực hiện hành động tương ứng,
  nếu không thì kiểm tra điều kiện tiếp theo. Nếu tất cả các điều kiện đều không đúng thì sẽ thực hiện phần else
...
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
  Việc sử dụng hàm giúp tái sử dụng code, làm cho chương trình ngắn gọn, dễ đọc và dễ bảo trì hơn.
  Khi cần thay đổi logic, chỉ cần sửa trong hàm 1 lần thay vì sửa ở nhiều chỗ khác nhau trong chương trình
